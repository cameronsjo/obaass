...
export class BackupToolProvider implements ToolProvider {
		...
		return {
			...
			inputSchema: z.object({
				count: z
					...
					.max(1000)
					.optional()
					.describe("Number of commits to show (default: 20, max: 1000)"),
				file: z
					.string()
					.optional()
					.describe("Filter to commits affecting this file path"),
			}),
			handler: async (params) => {
				const { count = 20, file } = params as {
					count?: number;
					file?: string;
				};
				// Coerce to a bounded positive integer before interpolating into `-n`.
				const n = Math.min(Math.max(1, Math.floor(count)), 1000);
				const format = "--format=%H%n%h%n%aI%n%s%n---";
				const args = ["log", format, `-n${n}`];
	...
	private backupShowFile(): ToolDefinition {
		return {
			...
			handler: async (params) => {
				...
				const content = await git(this.vaultPath, [
					"show",
					`${commit}:${path}`,
				]);
	...
	private backupDiff(): ToolDefinition {
		return {
			name: "backup_diff",
			description:
				"Show the diff for a file between two commits, or a commit and the current state.",
			inputSchema: z.object({
				commit: z.string().describe("Git commit hash to diff from"),
				path: z
					.string()
					.optional()
					.describe("File path to diff (omit for all changes in commit)"),
				compareTo: z
					.string()
					.optional()
					.describe("Compare to this commit (default: parent commit)"),
			}),
			handler: async (params) => {
	...
	private backupRestore(): ToolDefinition {
		return {
			name: "backup_restore",
			description:
				"Restore a file from a previous git commit. Overwrites the current version.",
			inputSchema: z.object({
	...
	private backupSnapshots(): ToolDefinition {
		return {
			...
			description:
				"List restic backup snapshots. A direct JSON API is not yet implemented in " +
				"obsidi-backup; this returns the web UI URL where snapshots can be viewed.",
			inputSchema: z.object({}),
			handler: async () => {
				return textResult(
					`Restic snapshots are available via the backup web UI: ${this.client.baseUrl}/ui/snapshots\nA direct JSON API is not yet implemented in obsidi-backup.`,
				);
			},
		};
	}
...
function parseGitLog(
	output: string,
): Array<{ hash: string; shortHash: string; date: string; message: string }> {
	const commits: Array<{
		hash: string;
		shortHash: string;
		date: string;
